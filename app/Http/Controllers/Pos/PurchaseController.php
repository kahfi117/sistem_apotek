...
class PurchaseController extends Controller
{
    public function PurchaseAll()
    {
        ...
        $allData = Purchase::with('supplier')
            ->select(
            'date',
            'supplier_id',
            'purchase_no',
            DB::Raw('count(product_id) as jumlah, sum(buying_price) as total_buying')
        )
            ->orderBy('date', 'desc')
            ->groupBy('purchase_no', 'date', 'supplier_id')
            ->get();
    ...
    {
        $purchase = Purchase::with('supplier', 'product', 'category')->where('purchase_no',$id)->first();

        $dataBuying = Purchase::where('purchase_no', $purchase->purchase_no)->get();

        return view('backend.pdf.purchase_pdf', compact('purchase', 'dataBuying'));
    }
